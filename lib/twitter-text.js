var TwitterText = (function () { 
    ...
    var regexes = {};
    var spaces = "\\s"
    regexes['spaces'] = new RegExp(spaces);
    var at_signs = '[@＠]';
    regexes['at_signs'] = new RegExp(at_signs);
    regexes['extract_mentions'] = new RegExp("(^|[^a-zA-Z0-9_])" + at_signs + "([a-zA-Z0-9_]{1,20})(?=(.|$))", "og");
    regexes['extract_reply'] = new RegExp("^(?:" + spaces + ")*" + at_signs + "([a-zA-Z0-9_]{1,20})", "og");
    ...
    var valid_domain = "(?:[^\\s][\\.-][^\\s]|[^\\W\\s]){1,}\\.[a-z]{2,}(?::[0-9]+)?";
    ...
    var valid_url = "( \
      ...
      ( \
        (https?:\\/\\/|www\\.) \
        (" + valid_domain + ") \
        (\\/" + valid_url_path_chars + "*" + valid_url_path_ending_chars + "?)? \
        (\\?" + valid_url_query_chars + "*" + valid_url_query_ending_chars + ")? \
        ) \
    )";
    
    regexes['valid_url'] = new RegExp(valid_url.replace(/[\s]/g,''), "ig");
